...
class JoinAfterScreen extends StatefulWidget {
  const JoinAfterScreen({Key? key}) : super(key: key);

  @override
  State<JoinAfterScreen> createState() => _InContentState();
}

class _InContentState extends State<JoinAfterScreen> {
                              ...
                              children: [
                                ...
                                Expanded(
                                  child: DropdownButtonFormField<Demande>(
                                    ...
                                    onChanged: (newValue) => setState(() => EXP_CER_SERIAL = newValue),
                                  ),
                                ),
                              ],
                            ),
...
//
  Future<void> _apisend() async {
    ...
    Success? success = (await ApiService().Setimportations(_EXPORT_COUNTRY!.name, _ORIGIN_COUNTRY!.name, _EXPORT_COUNTRYa!.name, _ORIGIN_COUNTRYa!.name, _TRANSIET_COUNTRY!.name,
        ENTERY_PORT, EXPECTED_ARRIVAL_DATE.text, SHIPPING_DATE.text, _controller.text, pdfFilePath, '', EXP_CER_SERIAL!.cERSERIAL.toString()));
  ...
  Future<void> checkd() async {
    List<String> variables = [EXP_CER_SERIAL != null ? EXP_CER_SERIAL!.cERSERIAL.toString() : ''];
    ...
    if (mounted) dialogInfo(context, text: 'عدد هجن : $status');
  }
}
